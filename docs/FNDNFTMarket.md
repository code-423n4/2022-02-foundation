...
```solidity
function acceptOffer(address nftContract, uint256 tokenId, uint256 minAmount) external nonpayable
```
...
| minAmount | uint256 | The minimum value of the highest offer for it to be accepted. This protects the seller in case an offer expires and allows another buyer to increase the highest available offer.
...
|---|---|---|
| auctionId `indexed` | uint256 | undefined |
| originalSellerAddress `indexed` | address | undefined |
| newSellerAddress `indexed` | address | undefined |
